...
Maps=$Home/Maps
Keys=$Home/Keys
Parents_R_ready_no_0s=$Home/Parents_R_ready_no_0s
Progeny_R_ready_no_0s=$Home/Progeny_R_ready_no_0s
...
if [ ! -e $Parents_R_ready_no_0s ] ; then mkdir $Parents_R_ready_no_0s; fi
if [ ! -e $Progeny_R_ready_no_0s ] ; then mkdir $Progeny_R_ready_no_0s; fi
...
predicted_parents=$Keys/usable_predicted_parents_double.csv  # This file is made from "Progeny_Parent_Finder.py"
progeny_key=$Keys/progeny_key.csv
cat $progeny_key | sed s'/KMERS/KMERS_Rready/'g > $Keys/progeny_key_Rready.csv #This allows the use of smaller Kmer Files
...
do 
    ...
    echo "${a}_KMERS_Stage2_Rready.txt">>${Keys}/${a}x${b}_Parents.txt
    echo "${b}_KMERS_Stage2_Rready.txt">>${Keys}/${a}x${b}_Parents.txt
...
do
    ...
    do
        echo "cp $Progeny_R_ready_no_0s/${file} $Working_Kmers_Progeny" >> $InterFiles/DeleteMe01.txt
    done
    ...
    do
        echo "cp  $Parents_R_ready_no_0s/${file} $Working_Kmers_Parents" >> $InterFiles/DeleteMe02.txt
    done
    ...
    # # A in hapmat means kmer is present. C means its missing.
