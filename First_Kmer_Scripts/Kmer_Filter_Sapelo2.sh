...
KMC_Progeny_Data=$Home/KMC_Progeny_Data/KMERS
InterFiles=$Home/InterFiles
KMC_Parent_Data=$Home/KMC_Parent_Data/KMERS
...
# Home=/scratch/drt83172/Wallace_lab/TallFescue/Data
# KMC_Progeny_Data=$Home/KMC_Progeny_Data
# InterFiles=$Home/InterFiles
# KMC_Parent_Data=$Home/KMC_Parent_Data
...
# python upper_lower_filter.py -k $KMC_Progeny_Data -l 6 -s $InterFiles/kmer_progeny_filter1.txt

# # Step 2
# # Filter parent files to only contain kmers that match with ones we have 
# >$InterFiles/DeleteMe.txt
# for i in $(ls $KMC_Parent_Data | cut -d . -f 1)
# do
# echo "python filter_one_dic_using_another.py -p $KMC_Parent_Data/${i}.txt -c $InterFiles/kmer_progeny_filter1.txt -s $Parent_stage_2/${i}_Stage2.txt" >> $InterFiles/DeleteMe.txt
# done
...
# for file in $(ls $KMC_Progeny_Data | cut -d . -f 1)
# do
# echo "python Getting_kmers_ready_for_R.py -i $KMC_Progeny_Data/${file}.txt -l $Final_Kmers/usefull_kmers.txt -s $Progeny_R_ready/${file}_Rready.txt" >> $InterFiles/WipeMe.txt
# done
